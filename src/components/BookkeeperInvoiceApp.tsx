...
export default function BookkeeperInvoiceApp({
  ...
  return (
    <div className="flex h-[calc(100vh-36px)]">
        ...
        <form onSubmit={handleSubmit} noValidate>
          ...
          <div className="px-6 py-4 space-y-6">
            ...
            <div className="form-section">
              ...
              <p className="text-[10px] text-slate-400 mb-3">These are saved in your browser automatically.</p>
            ...
            <div className="form-section">
              ...
              <div className="mb-3">
                <label className="block text-[10px] font-medium text-slate-600 mb-1">Select from saved clients</label>
                <select
              ...
              <div className="grid grid-cols-4 gap-3 mb-3">
                <div className="field col-span-2">
                  ...
                  <input
                    ...
                    className={selectedClientId !== null ? 'bg-slate-50 cursor-default' : ''}
                  />
                </div>
                <div className="field">
              ...
              <div className="grid grid-cols-4 gap-3">
                ...
                <div className="field">
                  ...
                  <input
                    readOnly
                    value={fmt(vatAmt)}
                    className="bg-slate-50 cursor-default font-mono"
                  />
                </div>
                <div className="field">
                  ...
                  <input
                    readOnly
                    value={fmt(total)}
                    className="bg-slate-50 cursor-default font-mono font-semibold text-indigo-700"
                  />
                </div>
              </div>
            </div>
            ...
            <div className="form-section">
              ...
              <textarea
                ...
                rows={3}
                className="w-full border border-slate-300 rounded px-2 py-1.5 text-xs focus:outline-none focus:ring-1 focus:ring-indigo-400"
      ...
      <div className="w-[45%] overflow-y-auto bg-slate-50 border-l border-slate-200">
        {/* Live preview */}
        <div className="bg-white m-4 rounded shadow-sm p-6 font-sans text-[11px]">
          <div className="flex justify-between mb-5">
            {/* Seller */}
            <div>
              <div className="font-bold text-base">{bk.name || <span className="text-slate-300">Your Name</span>}</div>
              {bk.address && <div className="text-slate-600">{bk.address}</div>}
              {(bk.postalCode || bk.city) && <div className="text-slate-600">{bk.postalCode} {bk.city}</div>}
              {bk.phone && <div className="text-slate-600">Puh: {bk.phone}</div>}
              {bk.email && <div className="text-slate-600">{bk.email}</div>}
              {bk.businessId && <div className="text-slate-500 mt-1">Y-tunnus: {bk.businessId}</div>}
              {bk.vatId && <div className="text-slate-500">ALV-tunnus: {bk.vatId}</div>}
            </div>
            {/* Invoice meta */}
            <div className="text-right">
              ...
              <table className="text-right ml-auto text-[10px]">
                <tbody>
                  <tr><td className="text-slate-400 pr-3">Laskunumero / Invoice No:</td><td className="font-bold">{invoiceNumber}</td></tr>
                  <tr><td className="text-slate-400 pr-3">Päivämäärä / Date:</td><td className="font-bold">{fmtDate(issueDate)}</td></tr>
                  <tr><td className="text-slate-400 pr-3">Eräpäivä / Due Date:</td><td className="font-bold">{fmtDate(dueDate)}</td></tr>
                  <tr><td className="text-slate-400 pr-3">Maksuehto / Payment Terms:</td><td className="font-bold">{paymentTerms}</td></tr>
                  <tr><td className="text-slate-400 pr-3">Laskutuskausi / Billing Period:</td><td className="font-bold">{fmtDate(periodStart)} – {fmtDate(periodEnd)}</td></tr>
                </tbody>
              </table>
            </div>
          </div>
          ...
          {/* Bill To */}
          <div className="mb-4">
            <div className="text-[9px] uppercase tracking-wide text-slate-400 mb-1">Laskutettava / Bill To</div>
            <div className="font-bold">{client.name || <span className="text-slate-300">Client Name</span>}</div>
            {client.address && <div className="text-slate-600">{client.address}</div>}
            {(client.postalCode || client.city) && <div className="text-slate-600">{client.postalCode} {client.city}</div>}
            {client.businessId && <div className="text-slate-500">Y-tunnus: {client.businessId}</div>}
            {client.vatId && <div className="text-slate-500">ALV-tunnus: {client.vatId}</div>}
          </div>
          ...
          <div className="flex justify-end mb-5">
            <table className="text-[10px]">
              <tbody>
                <tr>
                  <td className="text-slate-400 pr-6 text-right">Veroton myynti / Tax base:</td>
                  <td className="text-right w-20">{fmt(amtEx)} €</td>
                </tr>
                <tr>
                  <td className="text-slate-400 pr-6 text-right">ALV {fmt(vatPct)} %:</td>
          ...
          {/* Payment box */}
          <div className="border border-slate-200 rounded p-3 text-[10px]">
            <div className="font-bold text-[11px] mb-2 border-b border-slate-100 pb-1">Maksutiedot / Payment Details</div>
            <div className="grid grid-cols-2 gap-1">
              <span className="text-slate-400">IBAN:</span>
              <span className="font-bold">{bk.iban ? formatIbanDisplay(bk.iban) : <span className="text-slate-300">—</span>}</span>
              <span className="text-slate-400">BIC/SWIFT:</span>
              <span className="font-bold">{bk.bic || <span className="text-slate-300">—</span>}</span>
              <span className="text-slate-400">Viesti / Message:</span>
              <span className="font-bold">Bookkeeping invoice</span>
              <span className="text-slate-400">Eräpäivä:</span>
          ...
          {notes && (
            <div className="mt-3 text-[10px]">
              <div className="text-[9px] uppercase tracking-wide text-slate-400 mb-1">Lisätiedot / Notes</div>
              <div className="text-slate-600 whitespace-pre-line">{notes}</div>
            </div>
          )}
        </div>
              ...
              <table className="w-full text-[10px]">
                <thead>
                  <tr className="bg-slate-50 border-b border-slate-200">
                    <th className="py-2 px-3 text-left font-semibold text-slate-600">Invoice No</th>
                    <th className="py-2 px-3 text-left font-semibold text-slate-600">Client</th>
                    <th className="py-2 px-3 text-left font-semibold text-slate-600">Date</th>
                    <th className="py-2 px-3 text-right font-semibold text-slate-600">ex-VAT</th>
                    <th className="py-2 px-3 text-right font-semibold text-slate-600">VAT</th>
                    <th className="py-2 px-3 text-right font-semibold text-slate-600">Total</th>
                    <th className="py-2 px-3 text-center font-semibold text-slate-600">PDF</th>
                    <th className="py-2 px-3 text-center font-semibold text-slate-600">Actions</th>
                  </tr>
                </thead>
                <tbody>
                  {history.map((inv) => (
                    <tr key={inv.id} className={`border-b border-slate-100 hover:bg-slate-50 transition-colors ${editingId === inv.id ? 'bg-amber-50 border-amber-200' : ''}`}>
                      ...
                      <td className="py-1.5 px-3 text-right font-mono text-slate-600">{inv.amountExVat.toFixed(2)}</td>
                      ...
                      <td className="py-1.5 px-3 text-center">
                        <div className="flex items-center justify-center gap-2">
                          <button
                            ...
                            className="text-indigo-600 hover:text-indigo-800 font-semibold text-[10px] px-1.5 py-0.5 rounded hover:bg-indigo-50"
