  ...
  return (
    <div className="min-h-screen bg-canvas">
      <div className="bg-white/95 backdrop-blur border-b border-slate-200 px-8 py-3.5 flex items-center gap-4 sticky top-0 z-20">
        <div>
          <h1 className="font-bold text-base text-slate-800">My VAT / Oma Vero</h1>
          <p className="text-[10px] text-slate-400">Your complete VAT position as bookkeeper + gig worker</p>
        </div>
        <div className="flex items-center gap-2 ml-auto">
          {years.map((yr) => (
            <button key={yr} onClick={() => navigate(yr, selectedClientId)}
              className={`px-3 py-1 rounded text-xs font-semibold border transition-colors ${
                yr === year ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-white text-slate-500 border-slate-200 hover:border-indigo-400'
              }`}>{yr}</button>
          ))}
        </div>
      </div>
      ...
      <div className="max-w-5xl mx-auto px-6 py-5">
        ...
        <div className="bg-white border border-slate-200 rounded-xl px-5 py-4 mb-5 flex items-center gap-4">
          <div className="shrink-0">
            <div className="text-xs font-bold text-slate-700 mb-0.5">My gig worker record</div>
          ...
          {hasGig ? (
            <div className="shrink-0 text-right">
              ...
              <div className="text-[10px] text-slate-400">Gig work VAT included</div>
            </div>
          ) : (
            <div className="shrink-0 text-right">
              <div className="text-[10px] text-amber-600 font-semibold">BK fees only</div>
              <div className="text-[10px] text-slate-400">Select above for full picture</div>
            </div>
          )}
        </div>
        ...
        {hasGig ? (
          <div className="grid grid-cols-4 gap-4 mb-5">
            <div className="bg-white border border-slate-200 rounded-xl p-4">
              <div className="text-[10px] text-slate-400 mb-1 font-medium uppercase tracking-wide">Total Output VAT</div>
              <div className="text-xl font-bold text-orange-600 font-mono">{fmt(annualTotalOut)} EUR</div>
              <div className="text-[10px] text-slate-400 mt-1">BK {fmt(bkAnnualVat)} + Gig {fmt(round2(gigAnnualOutputVat + gigAnnualSubsOutputVat))}</div>
            </div>
            <div className="bg-white border border-slate-200 rounded-xl p-4">
              <div className="text-[10px] text-slate-400 mb-1 font-medium uppercase tracking-wide">Total Input VAT</div>
              <div className="text-xl font-bold text-green-700 font-mono">{fmt(annualTotalIn)} EUR</div>
              <div className="text-[10px] text-slate-400 mt-1">Workers {fmt(gigAnnualWorkerInputVat)} + Exp {fmt(gigAnnualExpenseInputVat)}</div>
            </div>
            ...
            <div className="bg-white border border-slate-200 rounded-xl p-4">
              <div className="text-[10px] text-slate-400 mb-1 font-medium uppercase tracking-wide">BK Service Income</div>
              <div className="text-xl font-bold text-indigo-700 font-mono">{fmt(bkAnnualExVat)} EUR</div>
              <div className="text-[10px] text-slate-400 mt-1">{bkInvoiceCount} invoice{bkInvoiceCount !== 1 ? 's' : ''}</div>
            </div>
          </div>
        ) : (
          <div className="grid grid-cols-4 gap-4 mb-5">
            <div className="bg-white border border-slate-200 rounded-xl p-4">
              ...
              <div className="text-[10px] text-slate-400 mt-1">ex-VAT - {bkInvoiceCount} invoice{bkInvoiceCount !== 1 ? 's' : ''}</div>
            </div>
            <div className="bg-white border border-slate-200 rounded-xl p-4">
              <div className="text-[10px] text-slate-400 mb-1 font-medium uppercase tracking-wide">VAT Collected (BK)</div>
              <div className="text-xl font-bold text-orange-600 font-mono">{fmt(bkAnnualVat)} EUR</div>
              <div className="text-[10px] text-slate-400 mt-1">Output VAT from clients</div>
            </div>
            <div className="bg-white border border-slate-200 rounded-xl p-4">
              <div className="text-[10px] text-slate-400 mb-1 font-medium uppercase tracking-wide">Total Invoiced</div>
              <div className="text-xl font-bold text-slate-800 font-mono">{fmt(bkAnnualIncVat)} EUR</div>
              <div className="text-[10px] text-slate-400 mt-1">inc. VAT paid by clients</div>
            </div>
        ...
        <div className="bg-white border border-slate-200 rounded-xl overflow-hidden mb-5">
          <div className="px-5 py-3 bg-slate-50 border-b border-slate-200">
            <h3 className="text-sm font-bold text-slate-700">Quarterly Oma Vero Reference — {year}</h3>
          ...
          <table className="w-full text-xs">
            <thead className="bg-slate-50 border-b border-slate-100">
              <tr>
                <th className="text-left px-5 py-3 font-semibold text-slate-600">Quarter</th>
                ...
                <th className="text-right px-5 py-3 font-semibold text-indigo-700">{hasGig ? 'Net Payable' : 'Remit to Vero'}</th>
              </tr>
            </thead>
            <tbody>
              {[1, 2, 3, 4].map((q) => {
                ...
                return (
                  <tr key={q} className={`border-t border-slate-100 ${!active ? 'opacity-40' : ''}`}>
                    <td className="px-5 py-3 font-semibold text-slate-800">
                      Q{q} <span className="font-normal text-slate-400">{Q_LABELS[q-1]}</span>
                      <div className="text-[10px] text-slate-400 font-normal">Deadline: {Q_DEADLINES[q-1]}.{String(year + (q === 4 ? 1 : 0)).slice(2)}</div>
                    </td>
                    ...
                    <td className={`px-5 py-3 text-right font-mono font-bold ${cq.net >= 0 ? 'text-indigo-700' : 'text-green-600'}`}>
            ...
            <tfoot className="border-t-2 border-slate-300 bg-slate-50">
              <tr>
                <td className="px-5 py-3 font-bold text-slate-700">Annual Total</td>
                ...
                <td className={`px-5 py-3 text-right font-bold font-mono text-lg ${annualNet >= 0 ? 'text-indigo-700' : 'text-green-600'}`}>{fmt(Math.abs(annualNet))} EUR</td>
              </tr>
            </tfoot>
          </table>
        </div>
        ...
        {bkInvoiceCount > 0 && (
          <div className="bg-white border border-slate-200 rounded-xl overflow-hidden mb-5">
            ...
            {bkQuarters.filter(q => q.invoices.length > 0).map((q) => (
              <div key={q.quarter} className="border-t border-slate-100">
                <div className="px-5 py-2 bg-slate-50 flex justify-between items-center">
                  <span className="text-[11px] font-bold text-slate-700">Q{q.quarter} · {Q_LABELS[q.quarter-1]}</span>
                  ...
                  <tbody>
                    {q.invoices.map((inv, idx) => (
                      <tr key={inv.id} className={idx % 2 === 0 ? 'bg-white' : 'bg-slate-50/60'}>
                        <td className="px-5 py-2 font-mono text-[11px] text-slate-500 w-48">{inv.invoiceNumber}</td>
                        <td className="px-3 py-2 text-slate-500 w-28">{fmtDate(inv.issueDate)}</td>
                        <td className="px-3 py-2 font-medium text-slate-800">{inv.clientName}</td>
                        <td className="px-3 py-2 text-right font-mono text-slate-600">{fmt(inv.amountExVat)}</td>
                        <td className="px-3 py-2 text-right font-mono text-orange-600 font-semibold w-24">{fmt(inv.vatAmount)} EUR</td>
                        <td className="px-5 py-2 text-right font-mono font-bold text-slate-800 w-28">{fmt(inv.totalIncVat)} EUR</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
        ...
        {hasGig && gigQuarters && (
          <div className="bg-white border border-slate-200 rounded-xl overflow-hidden mb-5">
            ...
            <table className="w-full text-xs">
              <thead className="bg-slate-50 border-b border-slate-100">
                <tr>
                  <th className="text-left px-5 py-2.5 font-semibold text-slate-600">Quarter</th>
              ...
              <tbody>
                {gigQuarters.map((q) => {
                  ...
                  return (
                    <tr key={q.quarter} className={`border-t border-slate-100 ${!active ? 'opacity-40' : ''}`}>
                      <td className="px-5 py-2.5 font-semibold text-slate-700">Q{q.quarter} <span className="font-normal text-slate-400">{Q_LABELS[q.quarter-1]}</span></td>
              ...
              <tfoot className="border-t-2 border-slate-200 bg-indigo-50">
                <tr>
        ...
        {/* How to file */}
        <div className="bg-indigo-50 border border-indigo-200 rounded-xl p-5 text-xs text-indigo-800">
          ...
          <div className="bg-white border border-indigo-100 rounded-lg p-3 font-mono text-[11px]">
