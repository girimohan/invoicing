...
export default function YELCalculator() {
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-xl font-bold text-slate-900">YEL Calculator</h1>
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-6 space-y-6">
        ...
        <div>
          ...
          <div className="flex items-center gap-4">
            ...
            />
            <div className="flex items-center border border-slate-300 rounded-lg overflow-hidden w-36">
        ...
        <div>
          <label className="flex items-start gap-3 cursor-pointer">
            ...
            />
            <div>
              <div className="text-sm font-medium text-slate-700">New entrepreneur discount</div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-6">
        ...
        {/* Mandatory check */}
        <div className={`flex items-center gap-3 p-3 rounded-lg mb-5 ${calc.isMandatory ? 'bg-indigo-50 border border-indigo-200' : 'bg-slate-50 border border-slate-200'}`}>
          <div className={`text-lg ${calc.isMandatory ? 'text-indigo-600' : 'text-slate-400'}`}>
            {calc.isMandatory ? '✔' : '—'}
          </div>
          <div>
            {calc.isMandatory ? (
              <span className="text-sm font-semibold text-indigo-800">YEL insurance is mandatory</span>
            ) : (
              <span className="text-sm font-semibold text-slate-500">YEL not mandatory at this income</span>
            )}
          ...
          <div className="space-y-3">
            <div className="grid grid-cols-2 gap-3">
              <div className="bg-slate-50 rounded-lg p-4">
                <div className="text-[10px] text-slate-500 uppercase tracking-wide font-medium">Monthly premium</div>
                <div className="text-2xl font-bold text-slate-900 mt-1">€{fmt(calc.monthlyPremium)}</div>
                <div className="text-xs text-slate-400 mt-0.5">per month</div>
              </div>
              <div className="bg-slate-50 rounded-lg p-4">
                <div className="text-[10px] text-slate-500 uppercase tracking-wide font-medium">Annual premium</div>
                <div className="text-2xl font-bold text-slate-900 mt-1">€{fmt(calc.annualPremium)}</div>
            ...
            <div className="grid grid-cols-3 gap-3 text-xs">
              <div className="bg-slate-50 rounded-lg p-3">
                <div className="text-slate-500 mb-1">YEL työtulo</div>
                <div className="font-semibold text-slate-800">€{fmt(calc.tyotulo)}</div>
              </div>
              <div className="bg-slate-50 rounded-lg p-3">
                <div className="text-slate-500 mb-1">Rate applied</div>
                <div className="font-semibold text-slate-800">{(calc.rate * 100).toFixed(3)}%</div>
              </div>
              <div className={`rounded-lg p-3 ${isNewEntrepreneur ? 'bg-green-50' : 'bg-slate-50'}`}>
                <div className="text-slate-500 mb-1">Discount saving</div>
                <div className={`font-semibold ${isNewEntrepreneur ? 'text-green-700' : 'text-slate-400'}`}>
      ...
      {/* What YEL affects */}
      <div className="bg-white rounded-xl border border-slate-200 p-6">
        <h2 className="text-sm font-semibold text-slate-700 mb-3">What YEL työtulo affects</h2>
        <div className="grid grid-cols-2 gap-2 text-xs text-slate-600">
          {[
            ...
            <div key={title} className="bg-slate-50 rounded-lg p-3">
              ...
              <div className="text-slate-400 mt-0.5">{note}</div>
            </div>
          ))}
        </div>
      </div>
