...
export default function VehicleTab({ clientId, year }: { clientId: number; year: number }) {
  ...
  return (
    <div className="space-y-5">
      <div>
        <h3 className="text-sm font-bold text-slate-700 mb-1">Vehicle & Mileage — {year}</h3>
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-4">
        <div className="text-xs font-semibold text-slate-600 mb-3">Vehicle used for deliveries</div>
      ...
      {isBike && (
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-xl border border-slate-200 p-5">
            <div className="text-xs font-semibold text-slate-700 mb-3">Personal Commute Deduction (Työmatkakulut)</div>
            <label className="flex items-start gap-3 cursor-pointer">
              ...
              <div>
                <div className="text-sm font-medium text-slate-700">This client commutes to work by bicycle/e-bike</div>
                <div className="text-xs text-slate-400 mt-0.5">Home → first pickup zone / delivery area → home</div>
              </div>
            </label>
          ...
          {/* Actual bike costs */}
          <div className="bg-white rounded-xl border border-slate-200 p-5">
            <div className="text-xs font-semibold text-slate-700 mb-1">Business Delivery Costs</div>
            ...
            <div className="grid grid-cols-2 gap-2 text-xs">
              {[
                ...
                <div key={i} className="bg-slate-50 rounded-lg p-2.5">
                  <div className="font-medium text-slate-700">{row.text}</div>
                  <div className="text-[10px] text-slate-400 mt-0.5">{row.note}</div>
                </div>
              ))}
            </div>
            <div className="mt-3 text-[10px] text-indigo-700 bg-indigo-50 border border-indigo-100 rounded p-2.5">
      ...
      {!isBike && (
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-3 gap-3">
            <div className="bg-white rounded-xl border border-slate-200 p-4">
              <div className="text-[10px] text-slate-500 uppercase tracking-wide font-medium">Business km logged</div>
              <div className="text-2xl font-bold text-slate-900 mt-1">{totalKm.toFixed(0)}</div>
            </div>
            <div className="bg-white rounded-xl border border-slate-200 p-4">
              <div className="text-[10px] text-slate-500 uppercase tracking-wide font-medium">Trips logged</div>
              <div className="text-2xl font-bold text-slate-900 mt-1">{trips.length}</div>
            </div>
            <div className="bg-white rounded-xl border border-slate-200 p-4">
              <div className="text-[10px] text-slate-500 uppercase tracking-wide font-medium">Commute rate (personal return)</div>
              <div className="text-lg font-bold text-slate-900 mt-1">{vehicleType === 'car' ? '€0.27' : '€0.13'}/km</div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
            <div className="px-5 py-3 border-b border-slate-100 flex items-center justify-between">
              <div>
                <div className="text-xs font-bold text-slate-700">Ajopäiväkirja — Trip Log {year}</div>
                <div className="text-[10px] text-slate-400">Required by Vero.fi for car/moped business deductions</div>
              </div>
              <button onClick={() => setShowAddTrip(!showAddTrip)}
                className="px-3 py-1.5 bg-indigo-600 hover:bg-indigo-600 text-white text-xs rounded-lg font-medium">
                + Add Trip
              </button>
            </div>
            ...
            {showAddTrip && (
              <div className="px-5 py-4 bg-indigo-50 border-b border-indigo-100 space-y-3">
                ...
                <div className="grid grid-cols-3 gap-3">
                  <div>
                    <label className="block text-[10px] text-slate-500 mb-1">Distance (km) *</label>
                  ...
                  <div>
                    <label className="block text-[10px] text-slate-500 mb-1">Odometer start (km)</label>
                  ...
                  <div>
                    <label className="block text-[10px] text-slate-500 mb-1">Odometer end (km)</label>
                ...
                <div>
                  <label className="block text-[10px] text-slate-500 mb-1">Purpose</label>
                ...
                <div className="flex gap-2">
                  <button onClick={addTrip}
                    className="px-4 py-1.5 bg-indigo-600 hover:bg-indigo-600 text-white text-xs rounded-lg font-medium">
                    Save Trip
                  </button>
                  <button onClick={() => setShowAddTrip(false)}
                    className="px-4 py-1.5 bg-white border border-slate-300 text-slate-600 text-xs rounded-lg font-medium">
                    Cancel
                  </button>
                </div>
              </div>
                ...
                <table className="w-full text-xs">
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {trips.map(t => (
                      <tr key={t.id} className="hover:bg-slate-50">
                        ...
                        <td className="px-3 py-2 text-right text-slate-400 font-mono text-[10px]">
                        ...
                        <td className="px-2 py-2">
                          <button onClick={() => setTrips(p => p.filter(x => x.id !== t.id))}
                            className="text-slate-300 hover:text-red-500 transition-colors text-base leading-none">×</button>
                        </td>
                      </tr>
                    ))}
                  </tbody>
                  <tfoot className="bg-slate-50 font-semibold">
                    <tr>
                      ...
                      <td className="px-3 py-2 text-right font-mono text-slate-800">{totalKm.toFixed(1)}</td>
                      <td colSpan={3}></td>
                    </tr>
                  </tfoot>
                </table>
